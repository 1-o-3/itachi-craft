...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // メタテーブル（最終更新日時を一元管理）
        await sql`
            CREATE TABLE IF NOT EXISTS itachi_meta (
        ...
        if (req.method === 'GET') {
            ...
            if (links.length === 0) {
                ...
                for (const item of initialData) {
                    await sql`
                        ...
                        VALUES (${item.id}, ${item.title}, ${item.url}, ${item.category}, ${item.desc}, ${item.image}, ${now});
                    `;
                }
                // 初期データ投入時の lastModified をセット
                await sql`
                    INSERT INTO itachi_meta (key, value) VALUES ('last_modified', ${now})
            ...
            // 最終更新日時をメタテーブルから取得
            const metaResult = await sql`SELECT value FROM itachi_meta WHERE key = 'last_modified';`;
            const lastModified = metaResult.rows[0]?.value || null;

            return res.status(200).json({ links, lastModified });
        }
        ...
        if (req.method === 'POST') {
            ...
            for (const link of links) {
                await sql`
                    INSERT INTO itachi_links (id, title, url, category, description, image, updated_at)
                    VALUES (${link.id}, ${link.title}, ${link.url}, ${link.category}, ${link.desc}, ${link.image}, ${link.updatedAt || now});
                `;
            }

            // 最終更新日時を「今」に更新（追加・編集・削除すべてで記録）
            await sql`
                INSERT INTO itachi_meta (key, value) VALUES ('last_modified', ${now})
                ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;
            `;

            return res.status(200).json({ ok: true, lastModified: now });
        }
